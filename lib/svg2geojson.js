...
function convertSVGToGeoJSON(doc, options={}) {
	...
	svg.$$.forEach( (el,i) => {
		if (options.layers && el['#name']==='g') {
			...
			addElementToLayer(el, layerByName[''],undefined);
		}
	});
	...
	for (var name in layerByName) {
		...
		const geo = {
			...
			features:layerByName[name].map(data => {
				...
				return {
					type:'Feature',
					properties:options.debug ? {id:data.coordinates.debugId} : null,
					geometry:data
				};
			})
		};
...
function addElementToLayer(el, layer, xform, gparent) {
	xform = combineTransform(el,xform);
	switch(el['#name']) {
		case 'g':        return addGroupToLayer(el, layer, xform);
		case 'path':     return addPathToLayer(el, layer, xform,gparent);
		case 'rect':     return addRectToLayer(el, layer, xform,gparent);
		case 'line':     return addLineToLayer(el, layer, xform,gparent);
		case 'circle':   return addCircleToLayer(el, layer, xform,gparent);
		case 'ellipse':  return addEllipseToLayer(el, layer, xform,gparent);
		case 'polygon':  return addPolygonToLayer(el, layer, xform,gparent);
		case 'polyline': return addPolylineToLayer(el, layer, xform,gparent);
...
function addGroupToLayer(el, layer, xform) {
	el.$$ && el.$$.forEach(e => { addElementToLayer(e,layer,xform,el) })
}

function addPathToLayer(el, layer, xform, gparent) {
	const c = attrs(el,'d');
	if (c.d) {
		const geo = addPathData(layer, xform, c.d, true);
		const pathStart = c.d.match(/^.+?\d.*?[\s,-].*?\d.*?(?=[\s,a-z-]|$)/i);
		addDebugId(geo, el, `path @ ${pathStart && pathStart[0]}`,gparent)
	}
}

function addRectToLayer(el, layer, xform, gparent) {
	...
	addDebugId(geo, el, `rect @ ${c.x},${c.y}`,gparent);
	layer.push(geo);
}

function addLineToLayer(el, layer, xform, gparent) {
	...
	addDebugId(geo, el, `line @ ${c.x1},${c.y1}`,gparent);
	layer.push(geo);
}

function addCircleToLayer(el, layer, xform, gparent) {
	...
	addDebugId(geo, el, `circle @ ${c.cx},${c.cy}`,gparent);
}

function addEllipseToLayer(el, layer, xform, gparent) {
	console.error("FIXME: ADD SUPPORT FOR ELLIPSES");
	process.exit(2);
}

function addPolygonToLayer(el, layer, xform, gparent) {
	...
	addDebugId(geo, el, `polygon @ ${pts[0]},${pts[1]}`,gparent);
	layer.push(geo);
}

function addPolylineToLayer(el, layer, xform, gparent) {
	const c = attrs(el,'points');
	if (!c.points) return;
	const nums = c.points.split(/[\s,]+/);
	const pts = [];
	...
	addDebugId(geo, el, `polyline @ ${pts[0]},${pts[1]}`,gparent);
	layer.push(geo);
}
...
function addDebugId(geo, el, fallback, gparent) {
	geo.coordinates.debugId = (gparent.$ && gparent.$.id) ? `#${gparent.$.id.value}` : fallback;
}
